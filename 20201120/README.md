...
키 입력 대기, 데이터 수신 대기 등 IO와 관련해서 코드가 특정라인(연산 메서드 호출 부분)에 멈춰있음.
...
int port = 12345;
// 해당 포트 번호로 서버 시작!
final Serversocket sercerSocket = new ServerSocket(port);
// 클라이언트의 접속을 대기
...
1. 시작 라인(start-line)
   ...
   2. 상태 라인(서버): 요청처리 결과에 대한 것
2. 메세지 헤더(message)
3. 메세지 바디: 전송할 데이터(ex.form에서 submit 눌렸을 때 전송되는)
...
* Request-URI: URI(Uniform Resource Idetifier) - 리소스
...
전송할 데이터.
`Content-Length`: 메세지 바디와 데이터 길이 바이트 수를 명시 해야함.
...
* Status-Code: 서버의 처리 결과 상태 값을 표현하는 수(ex.404에러 페이지) 
* Reason-Phrase: 상태 값을 표현하는 문자열 (ex. Page Not Found)
...
2. 응답해줄 파일에서 mime type(i.e. text/html, image/png)을 반환해주는 메서드
   1. URLConnection.guessCountTypeFromName(경로)
